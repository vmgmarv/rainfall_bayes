...
if __name__ == "__main__":
    ...
    min_rain = 1.5
    days_landslide = pd.Timedelta(days=5,minutes=0)
    gap = pd.Timedelta(days=1, hours = 2)
    ###############################################################################
    ...
    #lines = lines[:-1]
    plt.vlines(x=lines,ymin=0,ymax=50, linestyle = '-.', color = 'green')
    ...
    final_result = final_result.drop_duplicates(subset='p', keep = 'first').sort_values(by='p',ascending=False)
